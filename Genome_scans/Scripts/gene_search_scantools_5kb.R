## This script takes a file of Scantools results, selects the top 20 highest FstWC results and using an annotated genome, identifies
...
## Own code added to include a for loop to improve the script.

library(tidyverse)
library(writexl)

# read in the SV scan data
scan_data <- read_tsv("~/Library/CloudStorage/OneDrive-Personal/Adaptation_to polyploidy_project/Genome_scans/Scantools/DIPTET_WS1_MS1_BPM.txt")
...
hits <- scan_data %>% slice_max(FstWC, n = 20)

# Print the top hits to a file
write_xlsx(hits, "Top20_scantools_FstWC_results.xlsx")


# find the nearest genes to the top 20 hits
hit_position <-hits$start[1:20]
hit_scaffold <- hits$scaff[1:20]
...
write.table(annotations_df, file = "Scantools_genes_within_5kb_top20hits.txt",sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
