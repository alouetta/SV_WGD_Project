## This script creates a list of the A.thaliana and A.lyrata orthologs for the A.arenosa genes identified using gene_search_scantools_5kb.R  
## for use in searches and and an excel file showing the Hit number, the position on the arenosa genome, the scaffold number, 
## the arenosa gene annotation and the corresponding ortholog name
...
# Read in the list of A. arenosa gene names created using gene_search_scantools_5kb.R script
arenosa_genes_df <- read_tsv("~/Library/CloudStorage/OneDrive-Personal/Adaptation_to polyploidy_project/Genome_scans/Scantools_genes_within_5kb_top20hits.txt")
...
write_xlsx(AT_orthologs_clean, "Scantools_results_AT_gene_search.xlsx")
# Create a text file of the thaliana gene names only for searches
AT_genes_only <- AT_orthologs_clean[, "A_thaliana_genes"]
write.table(AT_genes_only, "Scantools_thaliana_orthos_top20_5kb.txt", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
write_xlsx(AL_orthologs_clean, "Scantools_results_AL_gene_search.xlsx")
# Create a text file of the lyrata gene names only for functional searches etc.
AL_genes_only <- AL_orthologs_clean[, "A_lyrata_genes"]
write.table(AL_genes_only, "Scantools_lyrata_orthos_top20_5kb.txt", row.names = FALSE, col.names = TRUE, quote = FALSE)