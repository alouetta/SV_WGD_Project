# Notes on using and installing Entropy
...
Created a text file (ploidy_inds.txt) with the ploidy of each sample by copying and pasting the ploidy column from the "WGD samples" excel file (in data folder).
Checked that txt file contains 159 lines using wc -l ploidy_inds.txt
...
Output files == wgd_arenosakxchainy.hdf5 (where x = k and y = run no.)
