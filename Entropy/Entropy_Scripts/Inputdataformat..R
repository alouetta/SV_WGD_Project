## Script to prepare input data for entropy in the right format for k = 9-11
# March 2019 -- Vivaswat Shastry

## USAGE: Rscript inputdataformat.R wgd_arenosa.fourfold_filtered_pruned.vcf
...
if(mult.files){
  ...
  a.mean.gl<-apply(a.gl, c(1,2), function(x){temp<-sum(10^(-0.1*unlist(x)));sum((10^(-0.1*unlist(x)))*(0:max(ploidy)))/temp})
}
