...
PCA plot PCA_wgd_arenosa_all.pdf is very busy and has too many labels so created a plot without labels PCA_wgd_arenosa_nolabels.pdf by deleting the line (geom_text_repel(aes(label=id), size=4, max.overlaps = 100, force=20, color="black")+) and then added 1 label on for each population manually using a zoomed in PCA_wgd_arenosa_all.pdf as a guide.